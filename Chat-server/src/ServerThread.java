					...
					if(server.userExists(to)){
						...
						while(loop){
							...
							if(answer.compareTo("yes")==0){
								JSONObject server_json = getJson("accept_file",username+ " accepted the file request",fileSender,username);
								server.respond(server_json);
								server_json = getJson("server_info","You have accepted the clients request",username,"server");
								output.writeUTF(server_json.toJSONString());
								loop = false;
							}
							else if(answer.compareTo("no")==0){
								JSONObject server_json = getJson("reject_file",username+" rejected the file.",fileSender,username);
								server.respond(server_json);
								server_json = getJson("server_info","You have declined the clients request",username,"server");
								output.writeUTF(server_json.toJSONString());
								loop = false;
							}
							else{
								JSONObject server_json = getJson("server_info","You need to input yes or no to the request: ",username,"server");
								output.writeUTF(server_json.toJSONString());
							}
